...
def compare_academic_history(ryan_data, major_objects, course_objects, bucket_objects):
  ...
  for i in major_objects.keys():
    ...
    # check their courses 
    for course_taken_id in history_ids:
      # print("course taken id ", course_taken)
      # see if it is in the course_ids set for this major 
      if course_taken_id in major_objects[i].course_ids:
        # print("course is in the major")
        hours_towards_major += int(course_objects[course_taken_id].hours)
      ...
      # if this class isnt in this major's course id's, check if it's in a bucket
        ...
        for bucket_id in bucket_objects.keys():
          if bucket_id in major_objects[i].bucket_ids:
            ...
            if course_taken_id in bucket_objects[bucket_id].course_ids:
              # print("course is in a bucket")
              found_in_a_bucket = True
              hours_towards_major += int(course_objects[course_taken_id].hours)
              # for kale printing function
              if(bucket_objects[bucket_id].num_hours > 0):
                bucket_objects[bucket_id].num_hours -= int(course_objects[course_taken_id].hours)
      ...
      #check if this current course could count as a senior elective
      # CE, COSC, BME, NE
      if(found_in_a_bucket == False):
        # print("not in a bucket or a required course!")
        # this class was not counted for!
        # check if its an upper division elective
        course_names = course_objects[str(course_taken_id)].names
        course_hours = course_objects[str(course_taken_id)].hours
        for course_name in course_names:
          if major_objects[str(i)].abbr == 'CE':
            if('CE3' in course_name or "CE4" in course_name):
              hours_towards_major += int(course_hours)
              break;
          elif major_objects[str(i)].abbr == 'CS':
            if("COSC3" in course_name or "COSC4" in course_name):
              hours_towards_major += int(course_hours)
              break;
          elif major_objects[str(i)].abbr == 'BME':
            if("BME3" in course_name or "BME4" in course_name):
              hours_towards_major += int(course_hours)
              break;
          elif major_objects[str(i)].abbr == 'NE':
            if("NE3" in course_name or "NE4" in course_name):
              hours_towards_major += int(course_hours)
              break;
